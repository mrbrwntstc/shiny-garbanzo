...
static f32 game_window_height = 1080;
...
void
render_quad_line (vec2 pos, vec2 size, vec4 color)
{
  vec2 points[4] = {
...
void
render_line_segment (vec2 start, vec2 end, vec4 color)
{
  glUseProgram (shader_default);
  glLineWidth (3);
...
void
render_aabb (f32 *aabb, vec4 color)
{
  vec2 size;
