...
static void
update_sweep_result (Hit *result, Body *body, usize other_id, vec2 velocity)
{
...
static void
update_sweep_result_static (Hit *result, Body *body, usize other_id, vec2 velocity)
{
...
static Hit
sweep_static_bodies (Body *body, vec2 velocity)
{
...
static Hit
sweep_bodies (Body *body, vec2 velocity)
{
  Hit result = { .time = 0xBEEF };
...
static void
sweep_response (Body *body, vec2 velocity)
{
...
physics_update (void)
{
  ...
  for (u32 i = 0; i < state.body_list->len; i++) {
    ...
    vec2 scaled_velociy;
    vec2_scale (scaled_velociy, body->velocity, global.time.dt * tick_rate);

    for (u32 j = 0; j < iterations; ++j) {
      sweep_response (body, scaled_velociy);
...
usize
physics_body_create (vec2 position, vec2 size, vec2 velocity, u8 collision_layer, u8 collision_mask, bool is_kinematic, On_Hit on_hit, On_Hit_Static on_hit_static)
{
...
Static_Body *
physics_static_body_get (usize index)
{
